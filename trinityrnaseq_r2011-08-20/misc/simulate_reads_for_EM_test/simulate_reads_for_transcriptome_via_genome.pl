...
#
#  --reads_per_transcript <string>   file containing format:  transcript_acc<tab>fragment_count
#
#  --genome <string>                 reference genome in fasta file format (required if --write_simulated_SAM)
#
# Optional:
#
...
&GetOptions( "gff3=s" => \$gff3_file,
             ...
             "reads_per_transcript=s" => \$reads_per_transcript_file,
...
unless ( ($gff3_file || $gtf_file) && $reads_per_transcript_file && $genome_file
...
main: {
    ...
    my %transcript_to_frag_count = &parse_frag_counts($reads_per_transcript_file);
    
    my %transcript_seqs;
    my $fasta_reader = new Fasta_reader($genome_file);
    %transcript_seqs = $fasta_reader->retrieve_all_seqs_hash();
    ...
    foreach my $contig (keys %$contig_to_gene_list_href) {
        ...
        foreach my $gene_id (@gene_ids) {
            ...
            foreach my $isoform_acc (keys %iso_coordsets) {
                ...
                my $frag_count = $transcript_to_frag_count{$isoform_acc};
                unless ($frag_count) {
                    print STDERR "Warning, no fragment count set for [$isoform_acc], skipping.\n";
                    next;
                }
                ...
                print STDERR "$frag_count reads simulated for $isoform_acc\n";
                my @paired_reads = $read_simulator->simulate_paired_reads($frag_count);
    ...
    my $cmd = "sort -k1,1 -k3,3 $trans_sam_outfile > $trans_sam_outfile.nameSorted.sam";
    &process_cmd($cmd);
    
    ## now, sort by coordinate.
    
    $cmd = "sort -k3,3 -k4,4n $trans_sam_outfile > $trans_sam_outfile.coordSorted.sam";
    &process_cmd($cmd);
    ...
    $cmd = "sort -k3,3 -k4,4n $genome_sam_outfile > $genome_sam_outfile.coordSorted.sam";
    &process_cmd($cmd);
