...
public:
  SeqChunk() {
    m_size = 100000;
    m_counter = 0;
    cout << "Initial resizing m_seq to " << m_size << endl;
  ...
  const char * Seq(long long i) const {
    if (i >= m_seq.isize()) {
      cout << "Error! i=" << i << " size=" << m_seq.isize() << endl;
      return NULL;
    }
    return &m_seq[i];
  }
  ...
  bool Add(long long & from, long long & to, const string & s) {
    ...
    if (len + m_counter >= m_size) {
      cout << "Overflow: " << len + m_counter << endl;
      return false;
    }
    ...
    if (m_seq.lsize() == 0) {
      cout << "Resizing m_seq to " << m_size << endl;
      m_seq.resize(m_size, 0);
    }
...
public:
  ...
  const char * Sequence(long long i) const {
    ...
    if (index > m_seq.isize()) {
      cout << "ERROR! " << index << " " << m_seq.isize() << endl;
    }
    const SeqChunk & c = m_seq[index];
    const char * pRet = c.Seq(i2);
    if (pRet == NULL) {
      cout << "Err: index=" << index << " i2=" << i2 << endl;
    }
    return pRet;
  ...
  void Add(const string & line) {
	...
	cout << "Resizing to " << m_kmerIndex + 100000 << endl;
	m_kmers.resize(m_kmerIndex + 100000);
	cout << "size=" << m_kmers.lsize() << endl;
      }
  ...
  void Setup() {
    ...
    cout << "Final size=" << m_kmers.lsize() << endl;
    Sort(m_kmers);
    cout << "done" << endl;
