...
void parse_target_kmers(string& filename, KmerCounter& kcounter, bool DS_mode);
bool contains_kmer(string& sequence, KmerCounter& kcounter, unsigned int kmer_length, int max_kmer_count, float min_kmer_entropy);
...
int run (int argc, char* argv[]) {
  ...
  usage << "usage: " << argv[0] << " --target target.fasta  --reads reads.fasta [options]" 
		<< endl << endl
		...
		<< "--max_kmer_count         maximum occurrence to use kmer for read recruitment" << endl
		<< "--min_kmer_entropy       minimum kmer entropy to use for recruitment (default: 1.5)" << endl
  ...
  KmerCounter kcounter(kmer_length);
  
  parse_target_kmers(targets_fa_filename, kcounter, DS_mode);
  ...
  while (fr.hasNext()) {
	...
	if (contains_kmer(seq, kcounter, kmer_length, max_kmer_count, min_kmer_entropy)) {
