...
string gumbelparout_file_name_)
{
	...
	f<<"Lambda\tLambda error\tK\tK error\tC\tC error\ta\ta error\ta_I\ta_I error\ta_J\ta_J error\tsigma\tsigma error\talpha\talpha error\talpha_I\talpha_I error\talpha_J\talpha_J error\tGapless a\tGapless a error\tGapless alpha\tGapless alpha error\tG\tCalculation time\tArrays for error calculation\n";
	f.precision(20);
	f<<
...
Sls::set_of_parameters &gumbel_params_)
{
		...
		double *BackgroundProbabilities1_sum=NULL;
		long int *BackgroundProbabilities1_sum_elements=NULL;
		...
		Sls::alp_data::read_RR(
		RR1_file_name_,
		BackgroundProbabilities1,
		BackgroundProbabilities1_sum,
		BackgroundProbabilities1_sum_elements,
		number_of_AA_RR1);

		double *BackgroundProbabilities2=NULL;
		double *BackgroundProbabilities2_sum=NULL;
		long int *BackgroundProbabilities2_sum_elements=NULL;
		...
		BackgroundProbabilities2_sum,
		BackgroundProbabilities2_sum_elements,
		number_of_AA_RR2);


		long int d_number_of_AA_smatr;

		Sls::alp_data::read_smatr(
		smatr_file_name_,
		ScoringMatrix_Njn,
		d_number_of_AA_smatr);

		if(number_of_AA_RR1!=d_number_of_AA_smatr||number_of_AA_RR2!=d_number_of_AA_smatr)
		{
			throw Sls::error("Number of letters is different for the scoring matrix and probabilities array\n",3);
		};
        ...
        gumbel_params_.a 
            = (gumbel_params_.a_I + gumbel_params_.a_J) * 0.5;

        gumbel_params_.a_error = (gumbel_params_.a_I_error
                                 + gumbel_params_.a_J_error)*0.5;
        
        gumbel_params_.alpha = (gumbel_params_.alpha_I
                               + gumbel_params_.alpha_J) * 0.5;

        gumbel_params_.alpha_error = (gumbel_params_.alpha_I_error
                                     + gumbel_params_.alpha_J_error) * 0.5;

		//------------------------------------------------------------------
		//the parameters are ready at this point; they are stored at "gumbel_params_" variable
                
		cout<<endl;
		cout << "Parameters estimation\n";
		cout << "Parameter value\terror\n";
		cout << "Lambda\t" << gumbel_params_.lambda << "\t" 
			<< gumbel_params_.lambda_error << endl;

		cout << "K\t" << gumbel_params_.K << "\t" << gumbel_params_.K_error
			<< endl;

		cout << "C\t" << gumbel_params_.C << "\t" << gumbel_params_.C_error
			<< endl;

		cout << "A_I\t" << gumbel_params_.a_I << "\t"
			<< gumbel_params_.a_I_error << endl;

		cout << "A_J\t" << gumbel_params_.a_J << "\t"
			<< gumbel_params_.a_J_error << endl;



		cout << "Sigma\t" << gumbel_params_.sigma << "\t" 
			<< gumbel_params_.sigma_error << endl;

		cout << "Alpha_I\t" << gumbel_params_.alpha_I << "\t" 
			<< gumbel_params_.alpha_I_error << endl;

		cout << "Alpha_J\t" << gumbel_params_.alpha_J << "\t" 
			<< gumbel_params_.alpha_J_error << endl;
		...
		cout << "Calculation time\t" << 	CurrentTime2-CurrentTime1
			<< " seconds" << endl;
		...
		if(ScoringMatrix_Njn)
		{
			for(i=0;i<NumberOfAAs;i++)
			{
				delete[]ScoringMatrix_Njn[i];
			};
			delete[]ScoringMatrix_Njn;
		};
...
vector<double>& pv_err_)
{
        ...
        long int size_tmp=p_values.size();
